...
if not os.path.exists("metadata_raw.pkl"):
    print ("Pickling amazon product metadata.")
    metadata_df_raw = pd.read_json("amazon_meta.json",lines=True)
    metadata_df_raw.to_pickle('metadata_raw.pkl')
    del metadata_df_raw

if not os.path.exists("reviews_raw.pkl"):
    print ("Pickling amazon reviews.")
    reviews_df_raw = pd.read_json("amazon_reviews.json",lines=True)
    reviews_df_raw.to_pickle('reviews_raw.pkl')
    del metadata_df_raw
