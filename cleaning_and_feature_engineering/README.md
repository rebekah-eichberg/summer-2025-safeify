...
- To skip steps 4-6 above, which take some time and require a CUDA-capable GPU, you can download the embedding data by running `src/download_embeddings.py`.

- In order to obtain the final dataset used in modeling, the necessary files can be downloaded by running the `src/download_traintestdata.py` script, which downloads the files generated by `combining_and_splitting.ipynb`.
