...
**Note:**  
- In order to run these noteboooks, you will need the raw amazon metadata and review data, which can be downloaded by running `src/download_split_finaldata.py`.

- In order skip steps 4-6 above, which take some time and require a CUDA-capable GPU, you can download the embedding data by running running `src/download_embeddings.py`.

- If order to obtain the final dataset used in modelling, the necessary files can be downloaded by running the `src/download_traintestdata.py` script, which downloads the files generated by `combining_and_splitting.ipynb`.
